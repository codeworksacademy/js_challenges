function interceptConsole() {
  ...
  function getTypeClass(value) {
    if (value instanceof Error) return 'console-error'
    ...
    for (let i = 0; i < args.length; i++) {
      ...
      if (typeof args[i] === 'string' && args[i].includes('%c')) {
        ...
        if (args[i] instanceof Error) {
          itemSpan.innerText = `${args[i].name} : ${args[i].message} ${args[i].lineNumber ? args[i].lineNumber : ''}`
          itemSpan.innerText += `\n -🔧- Check dev tools for more details`
        } else if (typeof args[i] === 'object') {
          // Pretty print objects and arrays
          itemSpan.innerText = JSON.stringify(args[i], null, 2)
        } else {
